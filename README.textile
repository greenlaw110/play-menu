...
* Support menu tag (not play template tag) to group your menus
...
_Menu(user):
    name: Manage user
    url: /admin/users
    context: admin # context is deprecated, use tags instead
    tags:
        - play
        - admin
...
p. Context is deprecated, use Tags instead
...
h3. <a>Use tags to group menu</a>

p. First you need to assign tags to menu items in your menu definition file "_menu.yml" if you use default implementation. Then you can define require menu.menu tag to display menus which are associated with specific tag. You can do it in two ways:

bc.. /* apply tags method 1: use controller to set the tag */
@Before
static void setMenuTag() {
    renderArgs.put("_menu_tag", "play");
}

/* apply tags method 2: assign tag in your template file */
<div id="admin-menu">#{menu.menu menu_tag: 'admin' /}</div>
<div id="user-menu">#{menu.menu menu_tag: 'user' /></div>

p. Note: menu items tagged with '_global' will always get displayed unless '_menu_no_global' options set to true
